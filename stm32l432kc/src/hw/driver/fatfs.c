...
static bool is_linked = false;
static bool cli_registered = false;
static FRESULT last_result = FR_NOT_READY;
...
bool fatfsInit(void)
{
  ...
  else
  {
    if (is_linked != true)
    {
      is_linked = (FATFS_LinkDriver(&SD_Driver, SDPath) == 0);
    }

    if (is_linked == true)
    {
      last_result = f_mount(&SDFatFs, (TCHAR const*)SDPath, 1);
      is_init = (last_result == FR_OK);
      ret = is_init;
    }
    else
    {
      last_result = FR_INVALID_DRIVE;
    }
  }
...
void cliFatfs(cli_args_t *args)
{
  ...
  if (args->argc == 1 && args->isStr(0, "test") == true)
  {
    ...
    pre_time = millis();
    fp_ret = f_open(&log_file, "0:/1.csv", FA_CREATE_ALWAYS | FA_WRITE | FA_READ);
    if (fp_ret == FR_OK)
    {
